...
OUTFILE=wcsim_beamlike_muon.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_beamlike_muon.${THECOUNTER}.log
...
ifdh cp -D ../WCSim/beamlike_muon.mac .
cd ..

export mac_through=build/beamlike_muon.mac
export TEMP_MACRO=build/beamlike_muon_temp.mac
export MACRO_LOCAL=beamlike_muon_temp.mac

#Adapt michel.mac file
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_beamlike_muon" >> $TEMP_MACRO
  else
