...
#SOURCEFILEZIP=wcsim_michel.tar.gz
#SOURCEFILEZIP=wcsim_throughgoing_new_multitank.tar.gz
#SOURCEFILEZIP=wcsim_pmtwiseqe.tar.gz
#SOURCEFILEZIP=wcsim_beamlike_new.tar.gz
SOURCEFILEZIP=wcsim_gst_new.tar.gz
...
GSTDIR=/pnfs/annie/persistent/users/mnieslon/genie/Vincent_Beam
OUTDIR=/pnfs/annie/persistent/users/mnieslon/wcsim/output/tankonly/wcsim_ANNIEp2v7_beam
...
GSTFILE=vincent_gst_${THEFILENUM}.root
OUTFILE=wcsim_beam_gst_${THEFILENUM}_${INFILE_OFFSETFACTOR}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_beam_gst_${THEFILENUM}_${INFILE_OFFSETFACTOR}.log
...
echo "input genie (gst) file is ${GSTDIR}/${GSTFILE}"
...
if [ ! -f ${GSTFILE} ]; then echo "gst file not found!!!"; exit 16; fi
...
cd build
ifdh cp -D ../WCSim/gst.mac .
cd ..

export mac_through=build/gst.mac
export TEMP_MACRO=build/gst_temp.mac
export MACRO_LOCAL=gst_temp.mac

#Adapt michel.mac file
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_beam_gst_${THEFILENUM}_${INFILE_OFFSETFACTOR}" >> $TEMP_MACRO
  else
...
echo "/mygen/gstdirectory ${PWD}/vincent_gst*root" >> macros/primaries_directory.mac
...
echo "/run/beamOn ${EVENTS_PER_WCSIM}" >> ${MACRO_LOCAL}   # will end the run as rqd if there are fewer events in the input file

RNDM=$(od -vAn -N2 -tu2 < /dev/urandom)
echo "Random number is ${RNDM}"
sed -i -e "1s/.*/\/WCSim\/random\/seed ${RNDM}/" macros/setRandomParameters.mac

sed -i -e "14s#.*#/WCSim/tuning/QEratio 1.40#" macros/tuning_parameters.mac
sed -i -e "10s#.*#/WCSim/tuning/teflonrff 1.00#" macros/tuning_parameters.mac
...
sed -i -e "22s#.*#/WCSim/tuning/PMTwiseQE 1#" macros/tuning_parameters.mac
