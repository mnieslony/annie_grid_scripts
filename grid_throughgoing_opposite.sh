...
OUTFILE=wcsim_throughgoing_muon_opposite_R${RUN}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.root
...
OUTLOG=wcsim_throughgoing_muon_opposite_R${RUN}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_throughgoing_muon_opposite_R${RUN}_Ratio${RATIO}_WBRatio${REF}" >> $TEMP_MACRO
