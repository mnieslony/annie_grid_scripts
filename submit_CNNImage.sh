...
if [ "$#" -ne 3 ]; then
      ...
      echo "Usage: ./submit_CNNImage.sh PMTFILES LAPPDFILES FILES_SUB"
...
do
	echo $PMTFILE
	echo $LAPPDFILE
	echo "./submit_ToolAnalysis_CNN_job.sh CNNImage_MC ${FILES_SUB} ${PMTFILE} ${LAPPDFILE} ${i}"
	./submit_ToolAnalysis_CNN_job.sh CNNImage_MC ${FILES_SUB} ${PMTFILE} ${LAPPDFILE} ${i}
	i=$((i+1))
