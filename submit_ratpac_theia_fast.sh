...
if [ "$#" -lt 0 ] || [ "$#" -gt 0 ]; then
    echo "Usage: ./submit_ratpac_theia_fast.sh"
    exit 1
fi
...
export QUEUE=long
export OUTDIR=FastNeutron_3pct_2021-08-04
export MACRO=${RATPAC_PATH}/theia-fastneutron.mac
...
export MACRO_FILE=${PNFS_PATH}/theia-fastneutron.mac

jobsub_submit -g -N 250 --memory=4000MB --expected-lifetime=${QUEUE} --group=$GROUP \
                    --resource-provides=usage_model=OPPORTUNISTIC \
                    --jobsub-server=https://fifebatch.fnal.gov:8443 \
                    -f $MACRO_FILE -f ${GRID_TAR_PATH}/ratpac_theia_fastneutron.tar.gz -f ${GRID_TAR_PATH}/geant4.10.01.02_for_grid.tar.gz\
                    -d OUTPUT /pnfs/annie/persistent/users/mnieslon/ratpac-theia/${OUTDIR}/ \
                    file:////annie/app/users/mnieslon/send_grid/ratpac-theia_grid_fast.sh theia_fastneutron ${MACRO_FILE} \
