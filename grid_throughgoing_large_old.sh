...
SOURCEFILEZIP=wcsim_michel_new.tar.gz
...
if [ "$#" -lt 5 ]; then
    echo "Usage: ./grid_throughgoing_large.sh RUN PART EV RATIO REF"
    exit 1
fi
...
OUTFILE=wcsim_throughgoing_R${RUN}p${PART}_Ratio${RATIO}_Ref${REF}.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_throughgoing_R${RUN}p${PART}_Ratio${RATIO}_Ref${REF}.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_throughgoing_muon_R${RUN}p${PART}_Ratio${RATIO}_Ref${REF}" >> $TEMP_MACRO
...
sed -i -e "10s#.*#/WCSim/tuning/teflonrff ${REF}#" macros/tuning_parameters.mac
