...
OUTFILE=wcsim_throughgoing_R${RUN}p${PART}_default.${THECOUNTER}.root
...
OUTLOG=wcsim_throughgoing_R${RUN}p${PART}_default.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_throughgoing_muon_R${RUN}p${PART}_default" >> $TEMP_MACRO
