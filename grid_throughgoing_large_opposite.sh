...
OUTFILE=wcsim_throughgoing_muon_opposite_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.root
...
OUTLOG=wcsim_throughgoing_muon_opposite_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_throughgoing_muon_opposite_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}" >> $TEMP_MACRO
  elif [[ $line == *"/mygen/vecfile"* ]]; then
    echo "/mygen/vecfile R${RUN}_throughgoing_opposite_nuance_${PART}.txt" >> $TEMP_MACRO
  else
