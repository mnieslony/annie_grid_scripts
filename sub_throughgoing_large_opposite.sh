...
do
	...
	do
		...
		do
			...
			jobsub_submit -N 1 --memory=2048MB --expected-lifetime=long --resource-provides=usage_model=DEDICATED,OPPORTUNIST -G annie file:///annie/app/users/mnieslon/send_grid/grid_throughgoing_large_opposite.sh ${run} ${part} ${ev} ${ratio} ${ref}
		done < $REFLIST
	done < $RATIOLIST
