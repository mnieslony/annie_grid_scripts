...
OUTFILE=wcsim_ambe_annri_default_port${PORT}_z${HEIGHT}.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_ambe_annri_default_port${PORT}_z${HEIGHT}.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_ambe_annri_default_port${PORT}_z${HEIGHT}" >> $TEMP_MACRO
  else
