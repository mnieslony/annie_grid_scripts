...
if [ "$#" -lt 5 ]; then
    echo "Usage: ./grid_throughgoing.sh RUN PART EV RATIO REF"
    exit 1
fi
...
#OUTFILE=wcsim_throughgoing_pmtwiseqe_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.root
OUTFILE=wcsim_throughgoing_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.root
...
OUTLOG=wcsim_throughgoing_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.log
...
ifdh cp -D ../WCSim/nuance-files/R*nuance* .
cd ..
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_throughgoing_muon_R${RUN}p${PART}_Ratio${RATIO}_WBRatio${REF}" >> $TEMP_MACRO
  elif [[ $line == *"/mygen/vecfile"* ]]; then
    echo "/mygen/vecfile R${RUN}_throughgoing_nuance_${PART}.txt" >> $TEMP_MACRO
  else
...
sed -i -e "16s#.*#/WCSim/tuning/QEratioWB ${REF}#" macros/tuning_parameters.mac
