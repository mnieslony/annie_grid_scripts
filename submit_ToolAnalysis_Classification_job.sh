...
if [ "$#" -ne 5 ]; then
      echo "Usage: ./submit_ToolAnalysis_Classificationjob.sh TOOLCHAIN TRANSFER_FILES_SUB PMTFILE LAPPDFILE JOBID"
...
if [[ "$TOOLCHAIN" == *"PrepareClassificationTraining"* ]]
then
    #OUTPUT_FOLDER=/pnfs/annie/persistent/users/mnieslon/cnnimage/mc/beamlike/
    OUTPUT_FOLDER=/pnfs/annie/persistent/users/mnieslon/classification/mc/beam_gst/
...
then
    ...
    echo "jobsub_submit -g --memory=4000MB --expected-lifetime=${QUEUE} --group=annie --disk=30GB \
            ...
            -f ${PMTFILE} \
            -f ${LAPPDFILE} \
            -f ${GRID_TAR_PATH}/ToolAnalysis_Classification.tar.gz \
            -d OUTPUT ${OUTPUT_FOLDER} \
            file://${SCRIPT_PATH}/ToolAnalysis_Classification_grid.sh ${PMTFILE_NOPATH} ${LAPPDFILE_NOPATH} configfiles/Classification/${TOOLCHAIN} ${var}"
...
then
    ...
    echo "jobsub_submit -g --memory=4000MB --expected-lifetime=${QUEUE} --group=annie --disk=30GB \
            ...
            -f dropbox://${PMTFILE} \
            -f dropbox://${LAPPDFILE} \
            -f dropbox://${GRID_TAR_PATH}/ToolAnalysis_Classification.tar.gz \
            -d OUTPUT ${OUTPUT_FOLDER} \
            file://${SCRIPT_PATH}/ToolAnalysis_Classification_grid.sh ${PMTFILE_NOPATH} ${LAPPDFILE_NOPATH} configfiles/Classification/${TOOLCHAIN} ${var}"
