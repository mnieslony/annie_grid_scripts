...
OUTFILE=wcsim_ambe_annri_HolderRef1.00_BSRef3.00_port${PORT}_z${HEIGHT}.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_ambe_annri_HolderRef1.00_BSRef3.00_port${PORT}_z${HEIGHT}.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_ambe_annri_HolderRef1.00_BSRef3.00_port${PORT}_z${HEIGHT}" >> $TEMP_MACRO
  else
