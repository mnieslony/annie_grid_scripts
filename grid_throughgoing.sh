...
if [ "$#" -lt 4 ]; then
    echo "Usage: ./grid_throughgoing.sh RUN EV RATIO REF"
    exit 1
fi
...
OUTFILE=wcsim_throughgoing_R${RUN}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.root
...
OUTLOG=wcsim_throughgoing_R${RUN}_Ratio${RATIO}_WBRatio${REF}.${THECOUNTER}.log
...
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_throughgoing_muon_R${RUN}_Ratio${RATIO}_WBRatio${REF}" >> $TEMP_MACRO
  elif [[ $line == *"/mygen/vecfile"* ]]; then
    echo "/mygen/vecfile R${RUN}_throughgoing_nuance.txt" >> $TEMP_MACRO
  else
