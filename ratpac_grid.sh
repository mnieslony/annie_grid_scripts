...
echo "ROOT_FILE_SINGLE: ${ROOT_FILE_SINGLE}"
...
cd $CONDOR_DIR_INPUT
#tar xzf ratpac-wbls.tar.gz
#tar xzf ratpac_for_grid-genie.tar.gz
#tar xzf ratpac_for_grid-photontag.tar.gz
#tar xzf ratpac_for_grid-edep.tar.gz
#tar xzf ratpac_for_grid_parentid.tar.gz
tar xzf ratpac_for_grid_ncapture2.tar.gz
tar xzf geant4.10.01.02_for_grid.tar.gz

# setup software
echo "Export RATPAC PATH, setting up ratpac and fife_utils"
export RATPAC_PATH=${CONDOR_DIR_INPUT}/rat-pac
setup_ratpac
setup fife_utils

ifdh cp -r ${CONDOR_DIR_INPUT}/$ROOT_FILE_SINGLE ${RATPAC_PATH}

# Copy macro file in output dir --> omit this copy process
#echo "Copy macro file to output dir"
#cp $CONDOR_DIR_INPUT/$MACRO_FILE_NOPATH $CONDOR_DIR_OUTPUT/$MACRO_FILE_NOPATH
...
$RATPAC_PATH/bin/rat -o ${CONDOR_DIR_OUTPUT}/$OUTPUT_FILE_NAME $CONDOR_DIR_INPUT/$MACRO_FILE_NOPATH > ${CONDOR_DIR_OUTPUT}/output_$MACRO_FILE_NOPATH
