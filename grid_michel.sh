...
OUTDIR=/pnfs/annie/persistent/users/mnieslon/wcsim/output/tankonly/wcsim_ANNIEp2v7_michel


if [ "$#" -lt 2 ]; then
    echo "Usage: ./grid_michel.sh RATIO WBRATIO"
    exit 1
fi
...
OUTFILE=wcsim_michel_Ratio${RATIO}_WBRatio${WBRATIO}.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_michel_Ratio${RATIO}_WBRatio${WBRATIO}.${THECOUNTER}.log
...
cd build
ifdh cp -D ../WCSim/michel.mac .
ifdh cp -D ../WCSim/michel_spectrum.dat .
cd ..

export mac_through=build/michel.mac
export TEMP_MACRO=build/michel_temp.mac
export MACRO_LOCAL=michel_temp.mac

#Adapt michel.mac file
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_michel_Ratio${RATIO}_WBRatio${WBRATIO}" >> $TEMP_MACRO
  else
...
sed -i -e "14s#.*#/WCSim/tuning/QEratio ${RATIO}#" macros/tuning_parameters.mac
...
sed -i -e "16s#.*#/WCSim/tuning/QEratioWB ${WBRATIO}#" macros/tuning_parameters.mac
