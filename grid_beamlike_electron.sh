...
OUTDIR=/pnfs/annie/persistent/users/mnieslon/wcsim/output/tankonly/wcsim_ANNIEp2v7_beamlike


#if [ "$#" -lt 2 ]; then
#    echo "Usage: ./grid_beamlike_electron.sh RATIO WBRATIO"
#    exit 1
#fi
...
OUTFILE=wcsim_beamlike_electron.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_beamlike_electron.${THECOUNTER}.log
...
cd build
ifdh cp -D ../WCSim/histogram_beamlike_e.dat .
ifdh cp -D ../WCSim/histogram_beamlike_mu.dat .
ifdh cp -D ../WCSim/beamlike_electron.mac .
cd ..

export mac_through=build/beamlike_electron.mac
export TEMP_MACRO=build/beamlike_electron_temp.mac
export MACRO_LOCAL=beamlike_electron_temp.mac

#Adapt michel.mac file
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_beamlike_electron" >> $TEMP_MACRO
  else
