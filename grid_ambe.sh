...
#SOURCEFILEZIP=wcsim_ambe.tar.gz
#SOURCEFILEZIP=wcsim_ambe_annri.tar.gz
...
OUTDIR=/pnfs/annie/persistent/users/mnieslon/wcsim/output/tankonly/wcsim_ANNIEp2v7_ambe


if [ "$#" -lt 2 ]; then
    echo "Usage: ./grid_ambe.sh PORT HEIGHT"
    exit 1
fi
...
OUTFILE=wcsim_ambe_annri_Ratio1.45_port${PORT}_z${HEIGHT}.${THECOUNTER}.root
#OUTFILE=wcsim_0.${PROCESSNUM}.root
OUTLOG=wcsim_ambe_annri_Ratio1.45_port${PORT}_z${HEIGHT}.${THECOUNTER}.log
...
cd build
ifdh cp -D ../WCSim/ambe*.mac .
cd ..

export mac_through=build/ambe_port${PORT}_z${HEIGHT}.mac
export TEMP_MACRO=build/ambe_temp.mac
export MACRO_LOCAL=ambe_temp.mac

#Adapt ambe.mac file
while read line; do
  if [[ $line == *"/WCSimIO/RootFile"* ]]; then
    echo "/WCSimIO/RootFile wcsim_ambe_annri_Ratio1.45_port${PORT}_z${HEIGHT}" >> $TEMP_MACRO
  else
...
sed -i -e "14s#.*#/WCSim/tuning/QEratio 1.45#" macros/tuning_parameters.mac
...
sed -i -e "16s#.*#/WCSim/tuning/QEratioWB 1.45#" macros/tuning_parameters.mac
