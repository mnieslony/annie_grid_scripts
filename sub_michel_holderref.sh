...
do
	...
	do
		...
		jobsub_submit -N 100 --memory=2048MB --expected-lifetime=long --resource-provides=usage_model=DEDICATED,OPPORTUNIST -G annie file:///annie/app/users/mnieslon/send_grid/grid_michel_holderref.sh ${ratio} ${wbratio}
	done < $WBRATIOLIST
done < $RATIOLIST
