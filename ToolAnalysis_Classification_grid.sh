...
ls -ltrh
echo "Contents of ToolAnalysis_Classification:"
ls ToolAnalysis_Classification
mv ToolAnalysis_Classification/MyToolAnalysis_Classification .
...
# setup software
export TOOLANALYSIS_PATH=${CONDOR_DIR_INPUT}/MyToolAnalysis_Classification
...
sed -i "5s#.*#InputFile ${PMTFILENAME}#" ${TOOLCHAIN_FILE}/LoadWCSimConfig
sed -i "5s#.*#InputFile ${LAPPDFILENAME}#" ${TOOLCHAIN_FILE}/LoadWCSimLAPPDConfig
...
sed -i "6s#.*#MCPMTVolCut 0#" ${TOOLCHAIN_FILE}/EventSelectorConfig
sed -i "6s#.*#Particle ID 13#" ${TOOLCHAIN_FILE}/MCRecoEventLoaderConfig
sed -i "9s#.*#MCIsMuonCut 0#" ${TOOLCHAIN_FILE}/EventSelectorConfig
sed -i "10s#.*#MCIsElectronCut 0#" ${TOOLCHAIN_FILE}/EventSelectorConfig
sed -i "4s#.*#Filename beam_gst_ANNIEp2v7_DigitThr0_${var}#" ${TOOLCHAIN_FILE}/StoreClassificationVarsConfig
...
${TOOLANALYSIS_PATH}/Analyse $TOOLCHAIN_FILE/ToolChainConfig > classification_gst_${var}.log
#exit

echo "Moving the output files to CONDOR OUTPUT:"

if [[ "$TOOLCHAIN_FILE" == *"PrepareClassificationTraining"* ]] 
then
    ...
    echo ifdh cp -r ${TOOLANALYSIS_PATH}/beam_gst_ANNIEp2v7* ${CONDOR_DIR_OUTPUT}
    ifdh cp -r ${TOOLANALYSIS_PATH}/beam_gst_ANNIEp2v7* ${CONDOR_DIR_OUTPUT}
    ifdh cp -r ${TOOLANALYSIS_PATH}/classification_gst_${var}.log ${CONDOR_DIR_OUTPUT}
fi
if [[ "$TOOLCHAIN_FILE" == *"ClusterFinder"* ]]
then
