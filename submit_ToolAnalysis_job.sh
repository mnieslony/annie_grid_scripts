...
then
    ...
    jobsub_submit -g --memory=4000MB --expected-lifetime=${QUEUE} --group=annie --disk=30GB \
            ...
            -f $FILENAME \
            -f ${GRID_TAR_PATH}/ToolAnalysis_DataDecoder_TrigOverlap_BeamStatus_improved4_daylight.tar.gz \
            -f $FILETRIGOVERLAP \
            -f $FILEBEAMSTATUS \
...
then
    ...
    jobsub_submit -g --memory=4000MB --expected-lifetime=${QUEUE} --group=annie --disk=30GB \
            ...
            --jobsub-server=https://fifebatch.fnal.gov:8443 -q \
            -f dropbox://$FILENAME \
            -f dropbox://${GRID_TAR_PATH}/ToolAnalysis_DataDecoder_TrigOverlap_BeamStatus_improved4_daylight.tar.gz \
